...
def getMatrix(dirname):
    ...
    m = len(pssmList)
    reMatrix = np.zeros((m, 1000))
    for i in range(m):
        matrix = readToMatrix(dirname + '/' + pssmList[i], 'psfm')
        matrix = autoNorm(matrix, 'psfm')
        ...
        WHmatrix = np.dot(matrix, binaryMatrix)  # L*10
        reMatrix[i, :] =getACC(WHmatrix,10)
...
def main():
    ...
    #np.savetxt("915-RFclassification-WHACC-LOO-probas_y.csv", loo_probas_y, delimiter=",")
    #np.savetxt("915-RFclassification-WHACC-LOO-test_y.csv", loo_test_y, delimiter=",")
    #
